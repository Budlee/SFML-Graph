...
void GraphLines::setPoint(float value)
{
    ...
    m_graphPoints.push_back(GraphPoint(screenHeight - (screenHeight *((100.0/currentMaxYValue *value)/100.0)),value));
    setPointLock.unlock();
}
...
void GraphLines::yValueChange(float newYValue)
{
    ...
    while (it != m_graphPoints.end())
    {
        ...
        it->setScreenPosition(screenHeight-(screenHeight * ((preVal * it->getValue())/100.0)));
        ++it;
    }
}